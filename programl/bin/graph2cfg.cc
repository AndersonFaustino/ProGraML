...
const char* usage =
    R"(Convert a ProgramGraph message to a Control and Data Flow Graph (CFG).

Usage:

    graph2dfg [--stdin_fmt={pb,pbtxt}]

The CFG format is a subset of a ProgramGraph which excludes data elements
...
int main(int argc, char** argv) {
  ...
  programl::graph::format::CFGBuilder builder;
  programl::ProgramGraph cfg = builder.Build(graph);
  programl::util::WriteStdout(cfg);
